...
export default function ProductCard({ product, user, onBuyNow, isBuying, isRazorpayLoaded, layout = 'grid' }: ProductCardProps) {
    ...
    return (
        <Card className="group h-full overflow-hidden border-border/50 bg-card transition-all hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1 duration-300 rounded-2xl flex flex-col relative">
            <button
                onClick={toggleFavorite}
                className="absolute top-3 right-3 z-20 p-2 rounded-full bg-white/80 backdrop-blur-sm shadow-sm hover:bg-white transition-all group-hover:opacity-100 opacity-100 sm:opacity-0 focus:opacity-100"
                title={isFavorited ? "Remove from favorites" : "Add to favorites"}
            >
                <Star className={`w-4 h-4 ${isFavorited ? 'fill-yellow-400 text-yellow-400' : 'text-muted-foreground'}`} />
            </button>
