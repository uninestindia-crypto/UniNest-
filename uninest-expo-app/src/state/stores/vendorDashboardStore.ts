...
type VendorDashboardState = {
  ...
  initializeDashboard: (vendorId: string | null) => Promise<void>;
  refreshDashboard: () => Promise<void>;
};

const initialState: Omit<VendorDashboardState, 'initializeDashboard' | 'refreshDashboard'> = {
  vendorId: null,
  loading: false,
  initialized: false,
  error: null,
...
async function hydrateDashboard(vendorId: string) {
  ...
  return {
    ...
    nudges,
    tierMetrics,
  } satisfies Omit<VendorDashboardState, 'vendorId' | 'loading' | 'initialized' | 'error' | 'initializeDashboard' | 'refreshDashboard'>;
}

export const useVendorDashboardStore = create<VendorDashboardState>((set, get) => ({
  ...initialState,
  initializeDashboard: async (vendorId) => {
    if (!vendorId) {
      set({ vendorId: null, initialized: true, error: 'Missing vendor context', ...initialState });
      return;
    }

    if (get().initialized && get().vendorId === vendorId) {
      return;
    }

    set({ loading: true, error: null, vendorId });

    try {
      const data = await hydrateDashboard(vendorId);
      set({ ...data, loading: false, initialized: true, error: null, vendorId });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to load dashboard';
      set({ ...initialState, vendorId, initialized: true, loading: false, error: message });
    }
  },
  refreshDashboard: async () => {
    ...
    try {
      const data = await hydrateDashboard(vendorId);
      set({ ...data, loading: false, error: null, initialized: true, vendorId });
    } catch (error) {
