...
1. Open the [MASTER_RESTORE_SCHEMA.sql](file:///d:/studio-uninest-deployed-main/scripts/MASTER_RESTORE_SCHEMA.sql) file.
...
## 4. Current Gaps
> [!WARNING]
> The `MASTER_RESTORE_SCHEMA.sql` reconstructs core tables (like `profiles` and `products`) based on their current structure. If you add new columns directly in the Supabase Dashboard without updating the script, those changes might be lost in a restoration. 
> **Always update the SQL files in your codebase when making schema changes.**
